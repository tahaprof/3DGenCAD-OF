  ...
  const renderFilterTags = () => {
    ...
    return (
      <div className="flex flex-wrap gap-2 items-center pb-4">
        <span className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium mr-2">Active Filters:</span>
        {tags}
  ...
  return (
    <div className="h-full flex flex-col font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white dark:text-slate-100">Asset Registry</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">Manage your complete inventory of industrial equipment.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`flex items-center p-2 rounded-xl text-sm font-medium transition-colors shadow-sm border
                ${isFilterOpen || activeFilterCount > 0
                  ? 'bg-slate-900 text-white dark:text-slate-100 border-slate-900'
                  : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-900'}`}
            >
              <Filter className="w-4 h-4 sm:mr-2" />
              <span className="hidden sm:inline">Filters</span>
              {activeFilterCount > 0 && (
                <span className={`ml-2 px-1.5 py-0.5 rounded-md text-[10px] font-bold
                  ${isFilterOpen || activeFilterCount > 0 ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white dark:text-slate-100' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 dark:text-slate-500'}`}>
                  {activeFilterCount}
                </span>
              )}
            </button>
              ...
              {isFilterOpen && (
                ...
                >
                  <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900">
                    <h3 className="font-semibold text-slate-900 dark:text-white dark:text-slate-100">Filter Assets</h3>
                    <button onClick={clearAllFilters} className="text-xs font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:text-white dark:text-slate-100">Reset</button>
                  </div>
                        ...
                        {FILTER_OPTIONS.stage.map(s => (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors
                              ${filters.stage.includes(s) ? 'bg-slate-800 text-white dark:text-slate-100 border-slate-800' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-900'}`}
                          >
                            {s}
                          </button>
                        ))}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {FILTER_OPTIONS.crit.map(c => (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors
                              ${filters.crit.includes(c) ? 'bg-slate-800 text-white dark:text-slate-100 border-slate-800' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-900'}`}
                          >
                            {c}
                          </button>
                        ))}
                          ...
                          {FILTER_OPTIONS.site.map(site => (
                            <label key={site} className="flex items-center gap-2 cursor-pointer group" onClick={(e) => { e.preventDefault(); toggleFilter('site', site); }}>
                              <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors
                                ...
                                {filters.site.includes(site) && <X className="w-3 h-3 text-white dark:text-slate-100" />}
                              </div>
                              <span className="text-sm text-slate-600 dark:text-slate-400 dark:text-slate-500 group-hover:text-slate-900 dark:text-white dark:text-slate-100">{site}</span>
                            </label>
                          ))}
                        </div>
                      </div>
                          ...
                          {FILTER_OPTIONS.system.map(sys => (
                            <label key={sys} className="flex items-center gap-2 cursor-pointer group" onClick={(e) => { e.preventDefault(); toggleFilter('system', sys); }}>
                              <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors
                                ...
                                {filters.system.includes(sys) && <X className="w-3 h-3 text-white dark:text-slate-100" />}
                              </div>
                              <span className="text-sm text-slate-600 dark:text-slate-400 dark:text-slate-500 group-hover:text-slate-900 dark:text-white dark:text-slate-100">{sys}</span>
                            </label>
                          ))}
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex-1 flex flex-col">
                          <label className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-1">Min</label>
                          <input
                        ...
                        <div className="flex-1 flex flex-col">
                          <label className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-1">Max</label>
                          <input
                            type="range" min="0" max="100"
          ...
          {canManageAssets && (
            <Link
              to="/assets/new"
              className="flex items-center bg-slate-900 hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-700 text-white dark:text-slate-100 px-4 py-2 rounded-xl text-sm font-medium transition-colors shadow-sm"
            >
        ...
        <div className="overflow-auto flex-1 relative">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50 dark:bg-slate-900/50 sticky top-0 z-10 backdrop-blur-sm">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider">TAG</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider hidden md:table-cell">Description</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider hidden lg:table-cell">Site / System</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider">Crit</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider">Stage</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider">Health</th>
              </tr>
            </thead>
              ...
              {filteredAssets.map((asset) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Link to={`/assets/${asset.id}`} className="text-sm font-bold text-slate-900 dark:text-white dark:text-slate-100 hover:text-blue-600 transition-colors">
                      {asset.tag}
                    </Link>
                    <div className="md:hidden text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">{asset.desc}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-400 dark:text-slate-500 font-medium hidden md:table-cell">
                    {asset.desc}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap hidden lg:table-cell">
                    <div className="text-sm text-slate-800 dark:text-slate-200">{asset.site}</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">{asset.system}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-400 dark:text-slate-500">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-slate-600 dark:text-slate-400 dark:text-slate-500 w-8">{asset.health}%</span>
                    </div>
                  </td>
                </tr>
              ))}
              {filteredAssets.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    ...
                    <h3 className="text-sm font-medium text-slate-900 dark:text-white dark:text-slate-100 mb-1">No assets found</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">Try adjusting your filters or search term.</p>
        ...
        {isNewAssetModalOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl shadow-xl z-50 overflow-hidden"
            >
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                <h2 className="text-lg font-bold text-slate-900 dark:text-white dark:text-slate-100">Register New Asset</h2>
                <button onClick={() => setIsNewAssetModalOpen(false)} className="p-2 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:bg-slate-800 rounded-lg">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-700 dark:text-slate-300">TAG Number *</label>
                    <input type="text" className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm" placeholder="e.g. P-205B" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Criticality</label>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm" placeholder="Brief description of the asset" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Site</label>
                    <select className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-white dark:bg-slate-900">
                      <option>Hassi Messaoud</option>
                      <option>Rhourde El Baguel</option>
                    </select>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-700 dark:text-slate-300">System</label>
                    <input type="text" className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm" placeholder="e.g. Gas Treatment" />
                  </div>
                </div>
              </div>
              <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex justify-end gap-3">
                <button onClick={() => setIsNewAssetModalOpen(false)} className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:text-white dark:text-slate-100 transition-colors">
                  Cancel
                </button>
                <button onClick={() => setIsNewAssetModalOpen(false)} className="px-4 py-2 bg-slate-900 hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-700 text-white dark:text-slate-100 rounded-xl text-sm font-medium transition-colors shadow-sm">
                  Save Asset
                </button>
              </div>
            </motion.div>
